...
# Primary database connection - use your Supabase connection string
...
PROD_DATABASE_URL=postgresql://USER:PASSWORD@aws-0-us-west-1.pooler.supabase.com:6543/postgres
```

Make sure:
- ✓ `DATABASE_TYPE=postgres` is set
- ✓ `DATABASE_URL` has your actual Supabase connection string
- ✓ The port is `6543` (Transaction mode) for better connection pooling
- ✓ You replaced the project reference and password
...
You can easily switch between different databases by changing the `DATABASE_URL`:
...
Just update `.env.local` and restart your server!
